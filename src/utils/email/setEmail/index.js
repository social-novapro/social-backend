...
async function replaceCurrentToEmail({ emailVerData }) {
    if (!emailVerData.replaceCurrent || !emailVerData.replaceEmail) return searchError("N026");
...
async function sendVerReplaceEmail({ userID }) {
    ...
    const foundEmailVer = await findCurUserVer({ userID });
    if (foundEmailVer.found != true) return searchError("N014"); 

    if (!foundEmailVer.data.replaceCurrent) return searchError("N021");

    // replaceEmail
    const replacedEmail = await replaceCurrentToEmail({ emailVerData: foundEmailVer.data });
